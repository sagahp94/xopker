...
export const AppDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [latestLogTimestamp, setLatestLogTimestamp] = useState<number | null>(null);
  const [missingDataDays, setMissingDataDays] = useState(0);
  const [isInitializing, setIsInitializing] = useState(true);
  const [isDataReady, setIsDataReady] = useState(false);
  const [initStep, setInitStep] = useState<'fetching' | 'preparing' | 'done'>('preparing');

  const calculateMissingDays = (lastTs: number | null): number => {
    if (!lastTs) return 0;
    const today = startOfDay(new Date());
    const lastDate = startOfDay(new Date(lastTs));
  ...
  const fetchAllData = useCallback(async () => {
    if (isDemoMode || user?.isDemo) {
      ...
      // Find latest entry timestamp across all activity logs and transactions
      const allTimestamps = [
        ...demoImports.map(i => i.timestamp),
        ...demoExports.map(e => e.timestamp),
        ...demoBorrows.map(b => b.timestamp),
        ...demoLogs.map(l => l.timestamp),
      ].filter(t => typeof t === 'number' && !isNaN(t) && t > 0);

      const latestTs = allTimestamps.length > 0 ? Math.max(...allTimestamps) : null;
      const missingDays = calculateMissingDays(latestTs);
      ...
      setLatestLogTimestamp(latestTs);
    ...
    try {
      ...
      const [
        ...
        getDocs(query(collection(db, 'activityLogs'), orderBy('timestamp', 'desc'), limit(1))).catch(() => null),
        getDocs(query(collection(db, 'exports'), orderBy('timestamp', 'desc'), limit(1))).catch(() => null),
        getDocs(query(collection(db, 'imports'), orderBy('timestamp', 'desc'), limit(1))).catch(() => null),
        getDocs(query(collection(db, 'borrowReturns'), orderBy('timestamp', 'desc'), limit(1))).catch(() => null),
      ]);
      ...
      // Calculate Latest Log Timestamp in Nhật Ký
      let maxTimestamp: number | null = null;
      const checkSnap = (snap: any) => {
        if (snap && !snap.empty && snap.docs.length > 0) {
          const d = snap.docs[0].data();
          if (d && typeof d.timestamp === 'number' && d.timestamp > 0) {
            if (maxTimestamp === null || d.timestamp > maxTimestamp) {
              maxTimestamp = d.timestamp;
            }
          }
        }
      };
      checkSnap(latestActivitySnap);
      checkSnap(latestExportSnap);
      checkSnap(latestImportSnap);
      checkSnap(latestBorrowSnap);

      const missingDays = calculateMissingDays(maxTimestamp);
      setLatestLogTimestamp(maxTimestamp);
      setMissingDataDays(missingDays);
