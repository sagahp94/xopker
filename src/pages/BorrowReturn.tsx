...
export const BorrowReturn: React.FC = () => {
  ...
  // Borrow State
  const [borrowDate, setBorrowDate] = useState<string>(() => format(new Date(), 'yyyy-MM-dd'));
  ...
  const handleBorrow = async (e: React.FormEvent) => {
    ...
    try {
      const selectedDate = borrowDate ? new Date(borrowDate + 'T00:00:00') : new Date();
      const now = new Date();
      selectedDate.setHours(now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
      const borrowTimestamp = selectedDate.getTime();

      if (isDemoMode) {
        demoStore.borrow(lenderId, borrowerId, borrowBagType, qty, user, borrowTimestamp);
        notifyDemoChange();
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-800">
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                  <Calendar className="w-4 h-4 text-indigo-500" /> Ngày Vay
                </label>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setBorrowDate(format(new Date(), 'yyyy-MM-dd'))}
                  ...
                  <span className="text-xs font-black text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/60 px-2.5 py-0.5 rounded-lg border border-indigo-200 dark:border-indigo-800">
                    {borrowDate ? format(new Date(borrowDate + 'T00:00:00'), 'dd/MM/yyyy') : ''}
                  </span>
                </div>
              </div>
              <input 
                ...
                className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-indigo-500 transition-all shadow-2xs cursor-pointer"
                required
              />
            </div>
