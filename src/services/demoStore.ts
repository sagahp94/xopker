...
class DemoStore {
  ...
  borrow(lenderId: string, borrowerOrBagType: string, bagTypeOrQty: any, qtyOrUser?: any, userObj?: User | null, customTimestamp?: number) {
    let bagTypeId: BagTypeID;
    let quantity: number;
    let user: User | null = null;
    let timestamp: number | undefined = customTimestamp;

    if (typeof bagTypeOrQty === 'number') {
      // (lenderId, bagTypeId, quantity, user, customTimestamp)
      bagTypeId = borrowerOrBagType as BagTypeID;
      quantity = bagTypeOrQty;
      user = qtyOrUser as User | null;
    } else {
      // (lenderId, borrowerId, bagTypeId, quantity, user, customTimestamp)
      bagTypeId = bagTypeOrQty as BagTypeID;
      quantity = Number(qtyOrUser) || 0;
      user = userObj || null;
    }

    return this.executeBorrow(lenderId, bagTypeId, quantity, user, timestamp);
  }
  ...
  executeBorrow(lenderId: string, bagTypeId: BagTypeID, quantity: number, user: User | null, customTimestamp?: number) {
    // Borrow adds stock to main department
    const current = this.inventory[bagTypeId] || 0;
    this.inventory[bagTypeId] = current + quantity;

    const recordTime = customTimestamp || Date.now();
